...
function CustomSelect({ value, onChange, options, placeholder = 'Select…' }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={label ? 'text-white/80' : 'text-white/25'}>
          {label || placeholder}
        </span>
        <ChevronDown
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="System health and business metrics"
      />
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          {
            label: 'Total MRR',
            value: `$${BUSINESSES.reduce((s, b) => s + b.mrr, 0)}`,
...
function BizForm({ initial = emptyBiz, onSubmit, onCancel }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...form,
      users: parseInt(form.users, 10) || 1,
      mrr: mrrMap[form.plan],
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormActions
        onCancel={onCancel}
        submitLabel={initial.name ? 'Update Business' : 'Add Business'}
      />
    </form>
  );
}
  ...
  const bizCols = [
    {
      ...
      render: (v, row) => (
        <div>
          ...
          {row.businessType && (
            <p className="text-xs text-white/35 mt-0.5">{row.businessType}</p>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      label: 'Plan',
      render: (v) => (
        <span className="text-indigo-300 text-xs font-medium">{v}</span>
      ),
    },
    { key: 'users', label: 'Users' },
    {
      key: 'mrr',
      label: 'MRR',
      render: (v) => <span className="font-semibold">${v}/mo</span>,
    },
    {
      key: 'joined',
      label: 'Joined',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Active',
            value: items.filter((b) => b.status === 'active').length,
            sub: 'subscribed',
          },
          {
            label: 'MRR',
            value: `$${items.reduce((s, b) => s + b.mrr, 0)}`,
            sub: 'monthly recurring',
          },
        ].map((s, i) => (
          <GlassCard key={s.label} delay={i * 0.08} className="p-4">
      ...
      <Modal
        open={!!editTarget}
        onClose={closeEdit}
        title={isNew ? 'Add Business' : 'Edit Business'}
      >
        <BizForm
          initial={isNew ? emptyBiz : editTarget}
          onSubmit={isNew ? create : (f) => update(editTarget.id, f)}
          onCancel={closeEdit}
        />
      </Modal>
      <DeleteModal
...
function UserForm({ initial = emptyUser, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormActions
        onCancel={onCancel}
        submitLabel={initial.name ? 'Update User' : 'Add User'}
      />
    </form>
  );
}
...
export function AdminUsers() {
  ...
  const userCols = [
    ...
    { key: 'name', label: 'Name' },
    {
      key: 'email',
      label: 'Email',
      render: (v) => <span className="text-xs text-white/45">{v}</span>,
    },
    {
      key: 'role',
      label: 'Role',
      render: (v) => <Badge status="active" label={v} />,
    },
    {
      key: 'business',
      label: 'Business',
      render: (v) => <span className="text-xs text-white/50">{v}</span>,
    },
    {
      key: 'id',
      label: '',
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <UserForm
          initial={isNew ? emptyUser : editTarget}
          onSubmit={isNew ? create : (f) => update(editTarget.id, f)}
          onCancel={closeEdit}
        />
      </Modal>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Active plans and billing overview"
      />
      <div className="grid grid-cols-3 gap-4">
        {[
          {
          ...
          {
            label: 'Suspended',
            value: items.filter((b) => b.status === 'suspended').length,
        ...
        <DataTable
          columns={[
            { key: 'name', label: 'Business' },
            {
              ...
              label: 'Plan',
              render: (v) => (
                <span className="text-indigo-300 font-medium text-xs">{v}</span>
              ),
            },
            {
              key: 'mrr',
            ...
            },
            {
              ...
              render: (_, row) => (
                <button
                  ...
                  className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors px-2 py-1 glass rounded-lg"
                >
                  Change Plan
                </button>
              ),
...
const logCols = [
  ...
  { key: 'action', label: 'Action' },
  {
    key: 'ip',
    label: 'IP',
    render: (v) => <span className="font-mono text-xs text-white/40">{v}</span>,
  },
  {
    key: 'time',
    label: 'Time',
    render: (v) => <span className="text-xs text-white/40">{v}</span>,
  },
  {
    key: 'level',
    label: 'Level',
...
export function AdminMonitoring() {
  const { items, deleteTarget, openDelete, closeDelete, remove } =
    useCrud(ACTIVITY_LOGS);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Activity logs and performance metrics"
      />
      ...
      <DeleteModal
        ...
        onClose={closeDelete}
        onConfirm={() => remove(deleteTarget.id)}
        itemName={`log entry by ${deleteTarget?.user}`}
      />
    </div>
  );
}
