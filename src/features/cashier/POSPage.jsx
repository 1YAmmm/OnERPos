...
import { Search, Plus, Minus, Trash2, X, ShoppingBag, CheckCircle2, Printer } from 'lucide-react';
...
export function POSPage() {
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-4 overflow-hidden">
            ...
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${
                  category === c ? 'bg-indigo-600 text-white' : 'glass text-white/50 hover:bg-white/6'
                }`}>
                {c}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-3 gap-3">
            {filtered.map((product, i) => (
              <motion.button
                ...
                className="glass rounded-xl p-4 text-left hover:bg-white/6 transition-colors group active:bg-indigo-500/10"
              >
                <div className="text-3xl mb-3">{product.image}</div>
                <p className="text-sm font-medium text-white/80 leading-tight mb-1 truncate">{product.name}</p>
                <p className="text-xs text-white/35 mb-2">{product.sku}</p>
                <div className="flex items-center justify-between">
                  <span className="text-base font-bold text-indigo-300">${product.price.toFixed(2)}</span>
            ...
            {filtered.length === 0 && (
              <div className="col-span-3 flex items-center justify-center py-16 text-white/25 text-sm">
                No products found
              </div>
            )}
          </div>
      ...
      {/* RIGHT: Cart */}
      <div className="w-full lg:w-80 xl:w-88 flex flex-col glass rounded-2xl overflow-hidden shrink-0">
              ...
              cart.map(item => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-indigo-300">${(item.price * item.qty).toFixed(2)}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <span className="w-6 text-center text-xs font-medium text-white/75">{item.qty}</span>
                    <button onClick={() => updateQty(item.id, 1)}
                      className="w-6 h-6 glass rounded-lg flex items-center justify-center text-white/50 hover:text-white/80 hover:bg-white/8 transition-colors">
        ...
        <div className="border-t border-white/8 px-4 py-4 space-y-3">
          <div>
            <label className="text-xs text-white/35 block mb-1.5">Discount ($)</label>
            <input
          ...
          <div className="space-y-1.5">
            {[
              ...
              { label: 'Discount', val: discountAmt > 0 ? `-$${discountAmt.toFixed(2)}` : '—' },
              { label: `Tax (${(TAX_RATE*100).toFixed(0)}%)`, val: `$${tax.toFixed(2)}` },
            ].map(r => (
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/8">
              ...
              <span className="text-xl font-bold text-indigo-300">${total.toFixed(2)}</span>
            </div>
          </div>
          <motion.button
            whileTap={{ scale: 0.97 }}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-base font-semibold text-white/85">Payment</h2>
                <button onClick={() => setShowCheckout(false)} className="text-white/30 hover:text-white/60">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="space-y-3 mb-5">
                ...
                <div>
                  <label className="text-xs text-white/40 block mb-2">Payment method</label>
                  <div className="grid grid-cols-3 gap-2">
                    {['Cash', 'Card', 'QR'].map(m => (
                      <button key={m} onClick={() => setPaymentMethod(m)}
                        className={`py-2.5 rounded-xl text-sm font-medium transition-all ${
                          paymentMethod === m ? 'bg-indigo-600 text-white' : 'glass text-white/50 hover:bg-white/6'
                        }`}>
                        {m}
                      </button>
                    ))}
          ...
          <>
            ...
            >
              <div className="text-center mb-5">
                ...
                <h2 className="text-base font-semibold text-white/85">Sale Complete</h2>
                <p className="text-xs text-white/35 mt-1">Transaction processed successfully</p>
              </div>
              <div className="glass rounded-xl p-4 mb-4 space-y-2">
                ...
                <p className="text-xs text-white/30 text-center pt-1">{paymentMethod} · {new Date().toLocaleString()}</p>
              </div>
              <div className="flex gap-2">
