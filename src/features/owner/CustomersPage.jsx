...
import { CustomersSkeleton } from '../../components/common/Skeleton';
...
function CustomerForm({ initial = emptyForm, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormActions
        onCancel={onCancel}
        submitLabel={initial.name ? 'Update Customer' : 'Add Customer'}
      />
    </form>
  );
}
...
export function CustomersPage() {
  ...
  const cols = [
    ...
    },
    {
      ...
      render: (v) => (
        <span className="font-semibold text-indigo-300">${v.toFixed(2)}</span>
      ),
    },
    {
      key: 'joined',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Total Revenue',
            value: `$${items.reduce((s, c) => s + c.totalSpent, 0).toFixed(2)}`,
            sub: 'from customers',
          },
        ].map((s, i) => (
