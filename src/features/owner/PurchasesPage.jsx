...
const emptyForm = { supplier: '', items: '', total: '', status: 'pending', date: new Date().toISOString().slice(0, 10), expectedDate: '' };
...
function POForm({ initial = emptyForm, onSubmit, onCancel }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit({ ...form, items: parseInt(form.items, 10), total: parseFloat(form.total) });
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Field label="Supplier Name">
        <Input required value={form.supplier} onChange={set('supplier')} placeholder="Bean Source Co." />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field label="Expected Date">
          <Input required type="date" value={form.expectedDate} onChange={set('expectedDate')} />
        </Field>
      </div>
      <Field label="Status">
      ...
      <FormActions onCancel={onCancel} submitLabel={initial.supplier ? 'Update Order' : 'Create Order'} />
    </form>
  );
}

export function PurchasesPage() {
  const { items, editTarget, deleteTarget, isNew, openCreate, openEdit, closeEdit, openDelete, closeDelete, create, update, remove } = useCrud(
    PURCHASE_ORDERS,
    makePoId
  );

  const cols = [
    { key: 'id', label: 'PO Number', render: v => <span className="font-mono text-xs text-indigo-300">{v}</span> },
    { key: 'supplier', label: 'Supplier' },
    { key: 'items', label: 'Items', render: v => `${v} items` },
    { key: 'date', label: 'Ordered', render: v => <span className="text-white/50 text-xs">{v}</span> },
    { key: 'expectedDate', label: 'Expected', render: v => <span className="text-white/50 text-xs">{v}</span> },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Purchase Management" subtitle="Supplier orders, approvals, and delivery tracking" />

      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Open Orders', value: open.length, sub: 'pending / in transit' },
          { label: 'Total PO Value', value: `$${totalOutstanding.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, sub: 'outstanding' },
          { label: 'Received', value: items.filter(p => p.status === 'received').length, sub: 'this session' },
        ].map((s, i) => (
      ...
      <GlassCard delay={0.2} className="p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button size="sm" onClick={openCreate}><Plus className="w-3.5 h-3.5" />New Order</Button>
        </div>
        <DataTable columns={cols} data={items} />
      </GlassCard>

      <Modal open={!!editTarget} onClose={closeEdit} title={isNew ? 'New Purchase Order' : 'Edit Purchase Order'}>
        <POForm
          initial={isNew ? emptyForm : editTarget}
          onSubmit={isNew ? (f) => create({ ...f, id: makePoId(items) }) : (fields) => update(editTarget.id, fields)}
          onCancel={closeEdit}
        />
      </Modal>
