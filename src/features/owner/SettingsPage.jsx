...
const TIMEZONES  = ['America/New_York', 'America/Los_Angeles', 'Europe/London', 'Asia/Manila', 'Asia/Tokyo'];
...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Settings" subtitle="Business configuration and preferences" />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Business Profile */}
        <GlassCard delay={0.1} className="p-5">
          <h3 className="text-sm font-semibold text-white/80 mb-4">Business Profile</h3>
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <Field label="Business Name">
              <Input value={profile.businessName} onChange={setP('businessName')} placeholder="My Business" />
            </Field>
            <Field label="Owner Name">
              <Input value={profile.ownerName} onChange={setP('ownerName')} placeholder="Your name" />
            </Field>
            <Field label="Email">
              <Input type="email" value={profile.email} onChange={setP('email')} placeholder="owner@business.com" />
            </Field>
            ...
            <Field label="Plan">
              <Input value={profile.plan} disabled className="opacity-50 cursor-not-allowed" />
            </Field>
        ...
        <div className="space-y-5">
          ...
          <GlassCard delay={0.15} className="p-5">
            <h3 className="text-sm font-semibold text-white/80 mb-4 flex items-center gap-2">
              <KeyRound className="w-4 h-4 text-white/40" />Change Password
            </h3>
            <form onSubmit={handleChangePw} className="space-y-4">
              <Field label="Current Password">
                <Input type="password" value={passwords.current} onChange={setPw('current')} placeholder="••••••••" />
              </Field>
              <Field label="New Password">
                <Input type="password" value={passwords.next} onChange={setPw('next')} placeholder="••••••••" />
              </Field>
              <Field label="Confirm New Password" error={pwError}>
                <Input type="password" value={passwords.confirm} onChange={setPw('confirm')} placeholder="••••••••" />
              </Field>
          ...
          <GlassCard delay={0.2} className="p-5">
            <h3 className="text-sm font-semibold text-white/80 mb-4">Notification Preferences</h3>
