...
import {
  Zap, Eye, EyeOff, ArrowRight, Building2, User, Mail, Lock,
    ...
    return (
      <div className="min-h-screen bg-[#080c14] flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-xl font-bold text-white/90 mb-2">Registration Successful!</h2>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        >
          <Link to="/" className="inline-flex flex-col items-center gap-3">
            ...
            <span className="text-lg font-semibold text-white/80">OnERPos</span>
          </Link>
          <h1 className="text-2xl font-bold text-white/90 mt-4 tracking-tight">Create your account</h1>
          <p className="text-sm text-white/35 mt-1">Set up your business workspace in minutes</p>
        </motion.div>
          ...
          <div className="flex items-center gap-2 mb-6">
            {steps.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2 flex-1">
                ...
                <span className={`text-xs font-medium transition-colors ${step === s.id ? 'text-white/70' : 'text-white/25'}`}>
                  {s.label}
                </span>
                {i < steps.length - 1 && (
                  <div className={`h-px flex-1 mx-1 transition-colors ${step > s.id ? 'bg-indigo-500/40' : 'bg-white/8'}`} />
                )}
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <span className={form.businessType ? 'text-white/80' : 'text-white/20'}>
                        {form.businessType || 'Select your business type'}
                      </span>
                      <ChevronDown className={`w-4 h-4 text-white/30 transition-transform ${typeOpen ? 'rotate-180' : ''}`} />
                    </button>
                    <AnimatePresence>
                      {typeOpen && (
                        <motion.div
                            ...
                            {BUSINESS_TYPES.map(type => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setFieldErrors(fe => ({ ...fe, businessType: '' }));
                                  setTypeOpen(false);
                                }}
                  ...
                  {fieldErrors.businessType && (
                    <p className="text-xs text-rose-400 mt-1">{fieldErrors.businessType}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full glass rounded-xl px-4 py-2.5 text-sm text-white/80 placeholder:text-white/20 outline-none transition-colors ${
                      fieldErrors.ownerName ? 'border-rose-500/60' : 'focus:border-indigo-500/40'
                    }`}
                  />
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full glass rounded-xl px-4 py-2.5 text-sm text-white/80 placeholder:text-white/20 outline-none transition-colors ${
                      fieldErrors.email ? 'border-rose-500/60' : 'focus:border-indigo-500/40'
                    }`}
                  />
                  {fieldErrors.email && (
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full glass rounded-xl px-4 py-2.5 pr-10 text-sm text-white/80 placeholder:text-white/20 outline-none transition-colors ${
                        fieldErrors.password ? 'border-rose-500/60' : 'focus:border-indigo-500/40'
                      }`}
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full glass rounded-xl px-4 py-2.5 pr-10 text-sm text-white/80 placeholder:text-white/20 outline-none transition-colors ${
                        fieldErrors.confirmPassword ? 'border-rose-500/60' : 'focus:border-indigo-500/40'
                      }`}
                    />
